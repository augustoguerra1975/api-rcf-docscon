...
# 1. CONFIGURAÇÃO DIRETA DO FIREBASE (Sem depender de arquivo externo)
firebase_creds = {
...
# Inicializa o Firebase usando o dicionário acima
...
async def receber_cota(request: Request):
    try:
        ...
        # O ID da cota na Docscon será o nome do documento no Firebase
        ...
        # Grava ou atualiza no Firebase
        db.collection("cotas_contempladas").document(cota_id).set(info_cota)

        return {"status": "sucesso", "cota": cota_id}
    except Exception as e:
        return {"status": "erro", "detalhes": str(e)}

@app.get("/")
def home():
    return {"RCF_Investimentos": "Sistema de Integração Ativo"}